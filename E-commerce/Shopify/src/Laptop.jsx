...
export default function Mobile() {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/products/category/laptop")
      .then((res) => res.json())
      .then((data) => {
        setItems(data || []);  
        setLoading(false);
...
const addToCart = async (product) => {
    ...
    return (
      <div className="min-h-screen py-6">
        <div className="mx-auto w-[92%] sm:w-11/12 md:w-5/6 lg:w-3/4">
          <h1 className="text-2xl sm:text-3xl font-bold mb-4">Mobiles</h1>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen py-6">
      ...
      <div className="mx-auto w-[92%] sm:w-11/12 md:w-5/6 lg:w-3/4">
        <h1 className="text-2xl sm:text-3xl font-bold">Laptops</h1>
